# Signal Handling in Namo

Namo implements robust signal handling to ensure data integrity during unexpected exits and to support responsive UI adjustments during terminal resizing.
...
To prevent data loss, Namo catches hangup and termination signals.
...
Namo typically runs with `ISIG` disabled in `termios` (see `posix.c`), meaning `Ctrl+C` does not send a `SIGINT`. Instead, it is read as a raw key code. This allows the editor to bind `Ctrl+C` to internal functions (like `insspace`) rather than crashing.
