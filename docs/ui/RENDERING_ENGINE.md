...
The display system in Namo is designed for high-performance rendering over terminal connections.

## 1. The Virtual Screen
Namo maintains an internal array of the entire screen state called `vscreen`. This is an array of `struct video`, which contains `video_cell` structures.
...
To save bandwidth, Namo never "clears and redraws" unless explicitly requested (via `Ctrl+L`). Instead, it uses `updateline()` to:
...
Colors are handled using ANSI escape sequences. Namo supports:
