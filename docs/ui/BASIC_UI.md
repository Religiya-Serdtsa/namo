...
Namo deliberately keeps the base interface purely ASCII so every terminal can reproduce it without extended glyphs. The diagram below reconstructs the default screen using the exact characters that `modeline()`, `draw_hint_row()`, and `mlwrite()` emit.
...
- The lamp system (`namo_set_lamp()` in `namo.c`) prints ASCII tokens (`--E`, `--W`, or blanks) so alerts stand out even without color.
- User overrides in `configs/namo/config` (`warning_format`, `error_format`) are expected to stay ASCII because the hint and modeline renderers treat each byte as a single column.
...
- The help sheet defined in `namo.c` also uses ASCII box rules (`=` and `-`) to mirror the look of the main UI.
