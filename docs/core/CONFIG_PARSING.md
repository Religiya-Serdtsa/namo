# INI Configuration Parsing in Namo

Namo uses a simple INI-style configuration format for both global settings and syntax highlighting rules. This document explains how the parsing is implemented and how to add new configuration options.
...
The parsing logic is primarily found in `namo.c` (for UI/Editor settings) and `highlight.c` (for syntax highlighting).
...
Namo searches for configuration files in the following order:
1. User config directory (typically `~/.config/namo/`)
2. User data directory (typically `~/.local/share/namo/`)
3. Relative path `configs/namo/` from the binary location.
...
1. Define the variable in the appropriate header (e.g., `struct namo_config` in `namo.h`).
2. Update the `parse_*_option` function in `namo.c` or `highlight.c` to recognize the new key.
