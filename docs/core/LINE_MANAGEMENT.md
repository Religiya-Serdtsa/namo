...
At its core, Namo represents text as a circular doubly-linked list of `line` structures. This document explains how text is stored and manipulated.
...
Namo is UTF-8 aware. While `l_text` stores raw bytes, functions like `linsert` and `backchar` use `unicode_to_utf8` and `utf8_to_unicode` to ensure cursor movement and editing respect multi-byte character boundaries.
