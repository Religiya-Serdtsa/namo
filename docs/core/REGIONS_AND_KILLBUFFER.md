...
uEmacs (and Namo) handles consecutive kill commands specially. If you kill multiple lines in a row, they are accumulated into a single kill buffer entry, so a single "Yank" will restore all of them. This is managed by the `thisflag` and `lastflag` variables checking for the `CFKILL` bit.
